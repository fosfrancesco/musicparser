...
################# Eisner slow algorithm for directed graphs ####################
def eisner_slow(scores):
    """
    Parse using Eisner's algorithm.
    """
    ...
    backtrack_eisner_slow(incomplete_backtrack, complete_backtrack, 0, N, 1, 1, heads)
...
def backtrack_eisner_slow(incomplete_backtrack, complete_backtrack, s, t, direction, complete, heads):
    """
    Backtracking step in Eisner's algorithm.
    - incomplete_backtrack is a (NW+1)-by-(NW+1) numpy array indexed by a start position,
    an end position, and a direction flag (0 means left, 1 means right). This array contains
    ...
    if complete:
        r = complete_backtrack[s][t][direction]
        if direction == 0:
            backtrack_eisner_slow(incomplete_backtrack, complete_backtrack, s, r, 0, 1, heads)
            backtrack_eisner_slow(incomplete_backtrack, complete_backtrack, r, t, 0, 0, heads)
            return
        else:
            backtrack_eisner_slow(incomplete_backtrack, complete_backtrack, s, r, 1, 0, heads)
            backtrack_eisner_slow(incomplete_backtrack, complete_backtrack, r, t, 1, 1, heads)
            return
    else:
        r = incomplete_backtrack[s][t][direction]
        if direction == 0:
            heads[s] = t
            backtrack_eisner_slow(incomplete_backtrack, complete_backtrack, s, r, 1, 1, heads)
            backtrack_eisner_slow(incomplete_backtrack, complete_backtrack, r+1, t, 0, 1, heads)
            return
        else:
            heads[t] = s
            backtrack_eisner_slow(incomplete_backtrack, complete_backtrack, s, r, 1, 1, heads)
